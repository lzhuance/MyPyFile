...
from readfile import readbrdc, readssrc
...
# 加入SSR改正的卫星位置计算
def satpos_ssr(time, teph, sat, ephs, gephs, corrs, opt, rs, dts):
    deph = np.zeros(3)
    corr = selcorr(teph, corrs)
    t1 = timediff(time, corr.time)
    t2 = timediff(time, corr.time)
    for i in range(3):
        deph[i] = corr.orb[sat][i] + corr.dorb[sat][i] * t1
    dclk = corr.clk[sat][0] + corr.clk[sat][1] * t2 + corr.clk[sat][1] * t2 * t2
...
def satpos(time, teph, sat, ephs, gephs, corrs, rs, dts, ephmode):
    if ephmode == 'BRDC':
        return ephpos(time, teph, sat, ephs, gephs, -1, rs, dts)
    elif ephmode == 'SSRA':   # 待添加ATX文件天线相位中心改正模块
        return satpos_ssr(time, teph, sat, ephs, gephs, corrs, 1, rs, dts)
    elif ephmode == 'SSRC':
        return satpos_ssr(time, teph, sat, ephs, gephs, corrs, 0, rs, dts)
    else:
        return rs, dts

# 计算teph历元下的卫星位置和钟差并存储至rss和dtss
def satposs(teph, ephs, gephs, corrs, ephmode):
    rss = np.zeros((NSAT, 6))
    ...
    #for sat in range(47,148):
        ...
        rs, dts = satpos(time, teph, sat, ephs, gephs, corrs, rs, dts, ephmode)
...
def process(ephmode, navfile, ssrcfile):
    init_global()  # 全局变量初始化
    time, savefile = gettime(navfile, ephmode)  # 获取首历元时间及输出sp3clk文件名
    ephs, gephs = readbrdc(navfile)             # 读取广播星历并存储至ephs和gephs
    corrs = readssr(ephmode, ssrcfile)          # 读取BNC SSR文件并存储至corrs
    ...
    for i in range(120):
        ...
        rss, dtss = satposs(time, ephs, gephs, corrs, ephmode)    # 计算该历元下的卫星位置和钟差并存储至rss和dtss
        week, sow = time2gpst(time)
...
if __name__ == '__main__':
    ...
    # SSRA：广播星历 + SSR APC
    # SSRC：广播星历 + SSR COM
    ephmode = 'BRDC'  # BDRC SSRA SSRC
    navfile = r'BRDM00DLR_S_20213530000_01D_MN.rnx'
    ssrcfile = r'SSRC00CNE0353A.21C'
    process(ephmode, navfile, ssrcfile)
