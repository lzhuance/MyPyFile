...
filename = filedialog.askopenfilename(filetypes=[('wlamb_no', '*.wlamb_no'), ('All Files', '*')])
print(filename)
...
c = np.ones([ aa,1],dtype=np.float32)*2.5
b = np.ones([ aa,1],dtype=np.float32)*-2.5
...
plt.xlabel('Epoch')
plt.savefig('HKWS MW.tiff', dpi=400)
plt.show()